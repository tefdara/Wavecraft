...
def generate_metadata(input_file, args):
    ...
    craft_data = get_craft_metadata(args)
...
def get_craft_metadata(args):
    
    normalization_metadata = stringify_dict(generate_normalization_metadata(args))
    filter_metadata = stringify_dict(generate_filter_metadata(args))
    trim_metadata = stringify_dict(generate_trim_metadata(args))
    fade_metadata = stringify_dict(generate_fade_metadata(args))
    audio_metadata = stringify_dict(generate_audio_settings_metadata(args))
    
    if args.operation == 'segment':
        seg_metadata = stringify_dict(generate_segmentation_metadata(args))
        metadata = [seg_metadata, normalization_metadata, filter_metadata, trim_metadata, fade_metadata, audio_metadata]
        metadata = join_metadata(metadata)
        return metadata
    elif args.operation == 'extract':
        fex_metadata = stringify_dict(generate_feature_extraction_metadata(args))
        metadata = [fex_metadata, audio_metadata]
        metadata = join_metadata(metadata)
        return metadata
    elif args.operation == 'decompose':
        decomp_metadata = stringify_dict(generate_decomposition_metadata(args))
        metadata = [decomp_metadata, audio_metadata]
        metadata = join_metadata(metadata)
        return metadata
    elif args.operation == 'beat':
        beat_metadata = stringify_dict(generate_beat_detection_metadata(args))
        ...
        pan_metadata = stringify_dict(generate_pan_metadata(args))
        metadata = [pan_metadata, audio_metadata]
        metadata = join_metadata(metadata)
        return metadata
    elif args.operation == 'split':
        split_metadata = stringify_dict(generate_split_metadata(args))
        metadata = [split_metadata, audio_metadata]
        metadata = join_metadata(metadata)
        return metadata
    elif args.operation == 'proxim':
        metadata = stringify_dict(generate_proximity_metric_metadata(args))
        return metadata

def stringify_dict(d, new_line=True):
    if new_line:
...
def generate_audio_settings_metadata(args):
    return {
...
def generate_segmentation_metadata(args):
    return {
...
def generate_feature_extraction_metadata(args):
    return {
        'fex_extractor': args.feature_extractor,
        'fex_flatten_dict': args.flatten_dictionary
    }

def generate_proximity_metric_metadata(args):
    return {
...
def generate_decomposition_metadata(args):
    return {
...
def generate_beat_detection_metadata(args):
    return {
        'beat_k': args.k
    }

def generate_filter_metadata(args):
    return {
        'filter_frequency': args.filter_frequency,
        'filter_type': args.filter_type
    }

def generate_normalization_metadata(args):
    return {
        'norm_level': args.normalisation_level,
        'norm_mode': args.normalisation_mode
    }

def generate_metadata_metadata(args):
    return {
        'meta_meta': args.meta,
        'meta_file': args.meta_file
    }

def generate_trim_metadata(args):
    return {
        'trim_range': args.trim_range,
        'trim_silence': args.trim_silence
    }

def generate_split_metadata(args):
    return {
        'split_points': args.split_points
    }

def generate_fade_metadata(args):
    return {
...
def generate_pan_metadata(args):
    return {
