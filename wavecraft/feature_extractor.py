    ...
    def extract(self):
        ...
        try:
            with warnings.catch_warnings(record=True) as w:
                D = librosa.stft(f_y, n_fft=self.args.n_fft)
                S_P = np.abs(D)**2
                ...
                # spectral features
                chroma_stft = librosa.feature.chroma_stft(S=S_M, sr=self.args.sample_rate)
                ...
                zcr = librosa.feature.zero_crossing_rate(f_y, hop_length=self.args.hop_size, frame_length=self.args.n_fft)
                ...
                tonnetz = librosa.feature.tonnetz(y=f_y, sr=self.args.sample_rate, chroma=chroma_stft)

                # rythm features
                onset_env = librosa.onset.onset_strength(S=S_P, sr=self.args.sample_rate, lag=2)
                tempogram = librosa.feature.tempogram(y=f_y, sr=self.args.sample_rate, onset_envelope=onset_env, win_length=self.args.window_length, hop_length=self.args.hop_size)
                fourier_tempogram = librosa.feature.fourier_tempogram(y=f_y, sr=self.args.sample_rate, onset_envelope=onset_env, win_length=self.args.window_length)
                ...
                if mfcc.shape[0] < n_mffc:
                    debug.log_info("<Padding> mfcc with zeros to match the number of rows of the PCA matrix")
                ...
                if fourier_tempogram.shape[0] < n_ftempo:
                    debug.log_info("<Padding> fourier tempogram with zeros to match the number of rows of the PCA matrix")
                ...
                if M.shape[0] < rn_mels:
                    debug.log_info("<Padding> mel spectrogram with zeros to match the number of rows of the PCA matrix")
                ...
                if(len(w) > 0):
                    for warn in w:
                        debug.log_warning(f'{warn.message}. <Line {warn.lineno} in file:> {warn.filename}') 
                    warns[self.file_name] = [warn for warn in w]
            
        except Exception as e:
            debug.log_error(f'Error processing {self.file_name}: {str(e)}', False)
